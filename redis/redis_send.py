...
<body>
<p><strong>检测到redis性能增加，请保持关注:</strong></p>
...
<tr>
  ...
  <td colspan="4"><strong>写入带宽使用率（%）</strong></td>
  <td colspan="4"><strong>读取带宽使用率</strong></td>
</tr>
<tr>
<td>8天前</td><td>2天前</td><td>1天前</td><td>cpu负载增加</td>
<td>8天前G</td><td>2天前</td><td>1天前</td><td>内存使用率增加（%）</td>
<td>8天前</td><td>2天前</td><td>1天前</td><td>写入带宽使用率增加</td>
<td>8天前</td><td>2天前</td><td>1天前</td><td>读取带宽使用率增加（%）</td>
</tr>
"""
    # html表格尾部
    htmlb = """
...
def get_alarm(name, date):
    sql = "select * from redis_alarm where name = '" + name + "' and date = '" + str(date) + "';"
...
def get_hosts():
    ...
    db = MySQLdb.connect("10.44.13.30","zabbix","zabbix","monitor")
    cursor = db.cursor()
    sql = "select `name` from redis_instanceid;"
    cursor.execute(sql)
    cur1 = cursor.fetchone()
    while cur1:
        hosts.append(cur1[0])
